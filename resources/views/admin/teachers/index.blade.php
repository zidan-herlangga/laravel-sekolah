...
<div class="card">
    <div class="card-header">
        <div class="d-flex justify-content-between align-items-center">
            <h3 class="card-title font-weight-bold"><i class="fas fa-chalkboard-teacher mr-2 text-success"></i>Guru & Staff</h3>
            <a href="{{ route('admin.teachers.create') }}" class="btn btn-primary btn-sm"><i class="fas fa-plus mr-1"></i> Tambah</a>
        </div>
    </div>
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><img src="{{ $teacher->photo ? Storage::url($teacher->photo) : 'https://via.placeholder.com/50x50?text=' . urlencode(substr($teacher->name,0,1)) }}" class="img-thumb" alt=""></td>
                        <td class="font-weight-semibold">{{ $teacher->name }}</td>
                        <td>{{ $teacher->position ?? '-' }}</td>
                        <td>
                            <span class="badge badge-status {{ $teacher->type === 'guru' ? 'badge-success' : 'badge-info' }}">
                                {{ ucfirst($teacher->type) }}
                            </span>
                        </td>
                        <td>{{ $teacher->order }}</td>
                        <td>
                            <a href="{{ route('admin.teachers.edit', $teacher) }}" class="btn btn-info btn-xs btn-flat"><i class="fas fa-edit"></i></a>
                            <form method="POST" action="{{ route('admin.teachers.destroy', $teacher) }}" class="d-inline" onsubmit="return confirm('Yakin hapus?')">
                                @csrf @method('DELETE')
